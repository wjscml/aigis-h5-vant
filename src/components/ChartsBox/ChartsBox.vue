<template>
  <div>
    <canvas id="container" width="345" height="260"></canvas>
  </div>
</template>
...
F2.Chart.plugins.register([Guide, Tooltip])
...
export default {
  ...
  mounted () {
    getDaysData({
      ...
      let sourceData = res.map(item => ({
        end: parseFloat(item.CLOSE),
        max: parseFloat(item.HIGH),
        min: parseFloat(item.LOW),
        start: parseFloat(item.OPEN),
        time: item.ftime
      }))
      // console.log(sourceData)
      var data = sourceData
      data = data.slice(0, 150) // 截取数据，只展示 200 条
      const BASIC_PRICE = this.currentPrice
      // 数据处理，按照时间排序
